...
server.all("/blogpost",function(req,res,next){
    ...
    const renderTypeDict = {list:RENDER_TYPE_LIST,show:RENDER_TYPE_SHOW_ARTICLE,edit:RENDER_TYPE_EDIT_ARTICLE};
    let method = req.method.toLowerCase();
            ...
            switch(op){
                case "list":
                    db.getBlogPost({},{title:true,time:true,type:true},10).then(function(result){
                        let renderObject = {renderType:renderTypeDict[op],docs:result};
                ...
                case "edit":
                    {
                        ...
                        if(id){ //存在文章ID，说明是查看某个文章的请求或加载某个文章修改页面的请求
                            ...
                            res.render("blogpost",{blogPost:JSON.stringify({renderType:renderTypeDict[op],doc:[]})});
                        }
                    }
                    break;
                default:
